#!/usr/bin/env python3
"""Phase A: Extract multi-layer TargetDiff embeddings for ALL 942 pockets.

Re-uses most logic from script 22, but runs on the FULL dataset.
Only processes pockets that DON'T already have multilayer_embeddings.npz.

Output per pocket: multilayer_embeddings.npz with keys layer_0..layer_9
"""
...
def main():
    ...
    # Filter out already-processed pockets
    if not args.force:
        needs_processing = [d for d in pocket_dirs if not (d / "multilayer_embeddings.npz").exists()]
        logger.info(f"Skipping {len(pocket_dirs) - len(needs_processing)} already-processed pockets")
        pocket_dirs = needs_processing

    logger.info(f"Processing {len(pocket_dirs)} pockets")
    ...
    for idx, pocket_dir in enumerate(pocket_dirs):
        ...
        try:
            ...
            if (idx + 1) % 50 == 0 or idx == 0:
                ...
                logger.info(f"[{idx+1}/{len(pocket_dirs)}] {family}: {n_mols} mols, OK")
