"""
scripts/utils/run_full_pipeline.py
────────────────────────────
...
Usage (debug mode, Mac CPU):
    python scripts/utils/run_full_pipeline.py --mode debug --device cpu
    python scripts/utils/run_full_pipeline.py --mode debug --dataset pdbbind --device cpu
"""
...
def prepare_data_pdbbind(splits_dir: Path, pdbbind_dir: Path, n_pockets: int = 5):
    ...
    ``scripts/pipeline/s01_prepare_data.py``.  Falls back to the ``test`` split; uses all
    ...
    if not labels_path.exists():
        raise FileNotFoundError(
            ...
            "Run scripts/pipeline/s01_prepare_data.py first."
        )
