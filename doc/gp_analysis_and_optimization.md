...
**代码**：`scripts/19_extract_encoder_embeddings.py`
...
### 16.4 已有 LOOCV 结果（Job 5020257 运行中）

| 策略 | 维度 | RMSE | Spearman ρ | R² |
|------|------|------|-----------|------|
| **mean** | 128 | **1.949** | **0.369** | **0.120** |
| mean+max | 256 | 1.955 | 0.364 | 0.115 |
...
> **注**：前 3 个策略结果与之前中断运行一致。mean+std 及后续策略仍在计算中。
> Job 5020257 预计 ~3.5h 完成全部 LOOCV + top-3 的 50× splits + TVT。

### 16.5 初步观察

1. **mean 仍为最优**（ρ=0.369），mean+max 仅差 Δρ=0.005
2. **高维拼接（256d）未带来提升**：说明当前数据量（942 样本）不足以支撑更高维特征
3. **max pooling（ρ=0.353）略差**：最大值聚合丢失了均值中的平均化降噪效果

### 16.6 脚本与产出

- 训练脚本：`scripts/21_train_gp_aggregation.py`
- SLURM 包装：`slurm/train_gp_aggregation.sh`
- 结果文件：`results/tier3_gp/p0plus_aggregation_results.json`（Job 完成后）
- 可视化：`results/tier3_gp/figures/p0plus_*.png`（Job 完成后）
