...
BayesDiff/
├── doc/
│   ├── 01_overall_plan.md         # 理论全景（已有）
│   ├── 04_opendata_plan.md        # 本文件：实操方案
│   └── 09_progress_log.md         # 开发日志
├── data/
...
│   └── ablation/
├── notebooks/
│   ├── test_pipeline.py       # Mac 端调试用
│   └── test_phase1_validation.py      # Phase 1 验证测试 (41 checks)
├── slurm/
...
> **独立脚本**：`scripts/pipeline/s04_train_gp.py`、`05_evaluate.py`、`06_ablation.py` 已创建，
...
- `scripts/scaling/s02_merge_shards.py`：合并分片产物
...
补充：并行执行使用 `scripts/scaling/s01_sample_shard.py` 作为分片包装器，内部调用原 `scripts/pipeline/s02_sample_molecules.py`，因此原采样代码保持不变。
...
## 2026-03-27 Tier 3 综合训练曲线与模型对比可视化

### 新增脚本

| 文件 | 用途 |
|------|------|
| `scripts/studies/tier3_training_curves.py` | FCFP4-2048 vs Encoder-128 综合可视化（7 图 + JSON） |
| `slurm/tier3_analysis.sh` | SLURM 提交脚本（cpu_short，Job 5038079） |

### 实验结论

在 Tier 3 数据集（FCFP4: N=932, Encoder: N=942）上完成了 FCFP4-2048 与 Encoder-128 的全面训练曲线对比。

**FCFP4-2048 vs Encoder-128 (10× 重复 60/20/20 划分)**：

| 模型 | Test RMSE | Test ρ | Test R² |
|------|:---------:|:------:|:-------:|
| FCFP4-2048 | 2.107±0.105 | 0.154±0.061 | 0.006±0.016 |
| **Encoder-128** | **1.983±0.112** | **0.352±0.068** | **0.103±0.039** |

**关键发现**：
1. Encoder-128 在 Test ρ 上比 FCFP4-2048 高 **2.3×**（0.352 vs 0.154），Test R² 高 **17×**
2. FCFP4 严重过拟合（Train ρ=0.988, Test ρ=0.154），Encoder 过拟合程度较低（Train ρ=0.887, Test ρ=0.352）
3. FCFP4 的 PCA PC1 仅解释 0.6% 方差，Encoder PC1 解释 36.4%——确认 Encoder 的潜空间结构更有意义
4. 10× 重复划分的 Test ρ 分布完全不重叠，Encoder 优势稳健

**可视化**：7 张图保存在 `results/tier3_gp/figures/tier3_analysis/`

详细分析见 `doc/06_gp_optimization.md` §17。
...
| Phase 0 | 环境 + 数据准备 | ✅ 完成 |
| Phase 1 | 核心模块实现 | ✅ 完成 |
| Phase 2 | HPC 批量采样 | ✅ 完成（Tier 3, N=932/942） |
| GP 基础 | Exact GP + RQ kernel + LOOCV | ✅ 完成（§12） |
| GP 优化 | Bayesian Optimization 200 trials | ✅ 完成（§10） |
| 表征诊断 | FCFP4 无信号确认 | ✅ 完成（R²=0.01） |
| P0 | TargetDiff 编码器嵌入 | ✅ 完成（ρ=0.369, GO）（§15） |
| P0+ | 聚合策略对比（7 种） | ✅ 完成（Δρ=+0.006, 非瓶颈）（§16） |
| 训练曲线可视化 | FCFP4 vs Encoder 综合对比 | ✅ 完成（§17） |
| 正则化实验 | DKL + Dropout + Weight Decay | ✅ 完成（DKL 无效，GP 本身最优）（§18） |
| P0++ | 多层嵌入融合 | ⬜ 待实施 |
| P1 | Vina 对接打分 | ⬜ 待实施 |
| P3 | 多表征融合 | ⬜ 待实施 |
| P4 | 端到端 GNN | ⬜ 待实施 |
