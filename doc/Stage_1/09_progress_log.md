...
## 2026-03-30: 预训练嵌入实验 & 50mol 消融研究

### Summary
系统评估了预训练 3D GNN 嵌入（Uni-Mol 512-dim, SchNet 128-dim）和化学指纹（FCFP4 2048-dim）作为 GP 输入。同时完成了 50mol encoder embeddings 的子采样消融。

**核心结论**:
1. 预训练嵌入均大幅逊于 TargetDiff Encoder baseline (Uni-Mol ρ=0.087 vs Encoder ρ=0.367)
2. 50mol encoder 与 tier3 ~5mol 性能相当 (ρ=0.399 vs 0.400)
3. 子采样 50→5 分子对性能无影响 (Δρ≈0.014)
4. FCFP4 化学指纹 (ρ=0.749) 远超所有 3D 嵌入方案

### Pre-trained Embedding Results (Job 5130267 — full 19 configs)

| Rank | Embedding | Dim | N | LOOCV ρ | Test ρ | Gap |
|------|-----------|-----|---|---------|--------|-----|
| #1 | **Encoder-128 (baseline)** | 128 | 942 | **0.367** | **0.362±0.054** | **0.525** |
| #2 | Encoder+SchNet→256d | 256 | 942 | 0.356 | 0.355±0.060 | 0.543 |
| #3 | Encoder+UniMol→PCA-64 | 64 | 932 | 0.264 | 0.289±0.058 | 0.638 |
| #9 | SchNet-128 | 128 | 942 | 0.096 | 0.118±0.063 | 0.561 |
| #11 | Uni-Mol-512 | 512 | 932 | 0.087 | 0.111±0.067 | 0.852 |
| #12 | FCFP4-2048 (ref) | 2048 | 932 | 0.114 | 0.106±0.066 | 0.879 |

### 50mol Subsample Ablation (A5)

| Subsample | LOOCV ρ | Test ρ |
|-----------|---------|--------|
| 50mol | 0.399 | 0.315±0.058 |
| 20mol | 0.385 | 0.326±0.086 |
| 10mol | 0.385 | 0.325±0.086 |
| 5mol | 0.385 | 0.325±0.086 |

### Scripts & Jobs
- `scripts/studies/embedding_unimol.py` — Uni-Mol extraction (Job 5123719, a100_chemistry)
- `scripts/studies/embedding_compare_all.py` — 17 configs comparison (6 figures)
- `scripts/studies/subsample_ablation.py` — Subsample ablation (Job 5127962, cpu_short)
- `scripts/studies/embedding_schnet.py` — SchNet extraction (Job 5130267, pending)

### 产物
- `results/tier3_gp/X_unimol_512.npy` (932×512) + 6 comparison figures
- `results/50mol_gp/ablation_results.json` (12 configs) + `05_ablation.png`
- `doc/08_embedding_plan.md` — 完整结果 & 结论

---

## 2026-03-27: 正则化实验 — Dropout + Weight Decay + Deep Kernel Learning

### Summary
在 Encoder-128 GP 模型上系统测试了 7 种正则化配置（Baseline GP、DKL ± Dropout 0.3/0.5 ± Weight Decay 0.01、GP+NoisePrior）。核心发现：**DKL 方案反而恶化泛化性能**，所有 DKL 变体的 Test ρ 均低于 Baseline GP。纯 GP 本身即为最优正则化器。

### Script & Execution

| Item | Details |
|------|---------|
| **Script** | `scripts/studies/regularization_study.py` |
| **SLURM job** | `slurm/regularization_study.sh`, Job 5039777, `cpu_short` partition |
| **Runtime** | ~2 min on CPU |
| **Output** | `results/tier3_gp/figures/regularization/` (5 PNGs + 1 JSON) |

### Key Results

| 配置 | Test ρ | 10× Test ρ | Gap (ρ) |
|------|:------:|:----------:|:-------:|
| Baseline GP | **0.347** | **0.352±0.068** | 0.556 |
| GP + NoisePrior | **0.347** | **0.352±0.069** | 0.555 |
| DKL + WD 0.01 | 0.290 | 0.197±0.070 | 0.708 |
| DKL + Dropout 0.3 | 0.288 | 0.236±0.074 | 0.688 |
| DKL (no reg) | 0.274 | 0.189±0.045 | 0.724 |
| DKL + Dropout 0.5 | 0.251 | 0.239±0.071 | 0.702 |
| DKL + D0.3 + WD | 0.236 | 0.263±0.065 | 0.746 |

### Key Findings

1. **DKL 加剧过拟合** — MLP(128→64→32) 在 N=942 小数据集上引入过多参数，即使 Dropout=0.5 也无法补偿
2. **Dropout + Weight Decay 组合反而最差** — DKL+D0.3+WD Test ρ=0.236，说明正则化手段无法解决 MLP 在小数据上的根本问题
3. **GP 天然正则化** — ExactGP 的 kernel 隐式限制函数复杂度，添加 MLP 层破坏了这一优势
4. **性能瓶颈在表征质量** — Encoder-128 仅解释 ~10% 方差，提升方向应为更丰富的嵌入而非模型正则化

---

## 2026-03-27: Tier 3 Training Curves & FCFP4 vs Encoder Comparative Visualization

### Summary
Created comprehensive 7-figure visualization comparing FCFP4-2048 and Encoder-128 GP models on Tier 3 data (N=932/942), including epoch-by-epoch training curves, learning curves on train/val/test splits, data distribution analysis, scatter plots, model comparison dashboard, residual & uncertainty calibration, and overfitting analysis.
...
| **SLURM job** | `slurm/tier3_analysis.sh`, Job 5038079, `cpu_short` partition |
| **Runtime** | ~2 min on CPU |
| **Output** | `results/tier3_gp/figures/tier3_analysis/` (7 PNGs + 1 JSON) |

### Key Results: FCFP4-2048 vs Encoder-128 (Single 60/20/20 Split)

| Metric | FCFP4 Train | FCFP4 Val | FCFP4 Test | Encoder Train | Encoder Val | Encoder Test |
|--------|:-----------:|:---------:|:----------:|:-------------:|:-----------:|:------------:|
...
### 10× Repeated Split Stability (mean ± std)

| Model | Train ρ | Val ρ | Test ρ | Test R² |
|-------|:-------:|:-----:|:------:|:-------:|
| FCFP4-2048 | 0.988±0.005 | 0.126±0.075 | 0.154±0.061 | 0.006±0.016 |
| Encoder-128 | 0.887±0.006 | 0.339±0.066 | 0.352±0.068 | 0.103±0.039 |

### Key Findings

1. **Encoder-128 generalizes far better** — Test ρ=0.347 vs 0.082 (4.2× improvement), Test R²=0.098 vs 0.012 (8.2×)
2. **Both models severely overfit** — Train ρ→0.98/0.90 but Test ρ→0.08/0.35; FCFP4 overfitting gap (0.90) much worse than Encoder (0.55)
3. **FCFP4 predictions collapse** — Val/Test predictions confined to [6.4, 7.2] pKd range (vs true range [1.3, 15.2])
4. **PCA variance explained** — FCFP4 PC1 explains only 0.6% vs Encoder PC1 explains 36.4%, confirming Encoder's richer latent structure
5. **Uncertainty poorly calibrated** — Both models' predicted σ tightly clustered (~1.8–2.1), many test residuals exceed predicted uncertainty

### Generated Figures

| Figure | File | Description |
|--------|------|-------------|
| 1 | `01_training_curves.png` | NLL loss, noise variance, output scale convergence (200 epochs) |
| 2 | `02_learning_curves.png` | Epoch-by-epoch RMSE/ρ/R² on train/val/test for both models |
| 3 | `03_data_distribution.png` | 9-panel: pKd histogram, split boxplots, mol counts, PCA colored by pKd, variance explained, embedding norms |
| 4 | `04_scatter_tvt.png` | 6-panel predicted vs true scatter (train/val/test × 2 models), colored by predicted σ |
| 5 | `05_model_comparison.png` | Bar chart + 10× repeated split boxplots for RMSE/ρ/R² |
| 6 | `06_residual_calibration.png` | Residual distributions, true vs predicted density, uncertainty calibration |
| 7 | `07_summary.png` | Overfitting gap (train ρ − test ρ) vs epoch + comprehensive metrics summary table |
...
### Summary
Audited all code against `doc/03_math_reference.md` formulas, fixed 4 math-code discrepancies, then diagnosed and fixed pipeline bugs identified in `doc/05_code_math_audit.md` (constant mu_pred, zero sigma2_gen).
...
Script: `notebooks/test_phase1_validation.py`
...
#### 3. `scripts/utils/run_full_pipeline.py`
...
**File**: `scripts/pipeline/s03_extract_embeddings.py`  
...
**File**: `scripts/pipeline/s05_evaluate.py`, `scripts/pipeline/s06_ablation.py`  
...
| `scripts/pipeline/s03_extract_embeddings.py` | 229 | +18 lines | Checkpoint auto-detection |
| `scripts/pipeline/s05_evaluate.py` | 254 | −4 lines | GP load fix |
| `scripts/pipeline/s06_ablation.py` | 288 | −3 lines | GP load fix |
...
```
OK  scripts/pipeline/s02_sample_molecules.py
OK  scripts/pipeline/s03_extract_embeddings.py
OK  scripts/pipeline/s04_train_gp.py
OK  scripts/pipeline/s05_evaluate.py
OK  scripts/pipeline/s06_ablation.py
...
| File | Change |
|------|--------|
| `scripts/pipeline/s04_train_gp.py` | Added `--device` arg, fixed `load_affinity_pkl()` |
| `scripts/pipeline/s05_evaluate.py` | Fixed `load_labels()` (pk field, pocket family aggregation) |
| `scripts/pipeline/s06_ablation.py` | Fixed `load_labels()` (same as 05) |
