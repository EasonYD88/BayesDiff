...
### Next: Sub-Plan 05 (Multi-Task Trunk Shaping)

使用 DKL Ensemble 作为 oracle head，探索 multi-task trunk shaping 能否改善 uncertainty quality。

---

## Sub-Plan 5: Multi-Task Trunk Shaping

**Status**: ❌ NO-GO (Tier 1 + Tier 2 均未通过 decision gate)
**Jobs**: Tier 1 `5887694`, Tier 2 `5914791`

### 动机

SP04 DKL Ensemble 达到 ρ=0.780 但 ρ_{|err|,σ}=0.091 偏低。假设：在 frozen embedding 和 oracle head 之间插入一个 multi-task trunk（回归 + 分类 + 排序），通过辅助任务信号 reshape 表示空间，可能改善 downstream UQ。

### 实现

| 文件 | 说明 |
|------|------|
| `bayesdiff/multi_task.py` | 核心模块：SharedTrunk, MultiTaskTrunk, MultiTaskHybridOracle, GroupedBatchSampler, within_group_ndcg |
| `scripts/pipeline/s20_augment_npz.py` | NPZ 增强：添加 PDB codes + cluster groups |
| `scripts/pipeline/s20_train_multitask_trunk.py` | 训练 pipeline（--no_trunk / v1 / v2） |
| `scripts/pipeline/s21_evaluate_multitask.py` | 评估 + Tier 1 go/no-go gate + 可视化 |
| `slurm/s20_multitask_tier1.sh` | Tier 1 SLURM (A5.0–A5.2 × 3 seeds) |
| `slurm/s20_multitask_tier2.sh` | Tier 2 SLURM (A5.3–A5.5 × 3 seeds) |

### 测试

| 测试集 | 数量 | 状态 |
|--------|------|------|
| Unit tests (`test_multi_task.py`) | 28 | ✅ 全通过 |
| Integration tests (`test_multi_task_integration.py`) | 6 | ✅ 全通过 |
| **总计** | **34** | **✅ 34/34** |

### 实验结果 (3-seed mean±std, CASF-2016 test set)

| 实验 | ρ | R² | RMSE | NLL | ρ_{|err|,σ} |
|------|---|-----|------|-----|-------------|
| **A5.0 无 trunk (SP4 baseline)** | **0.780±0.007** | **0.608±0.009** | **1.359±0.016** | **1.786±0.031** | **0.091±0.020** |
| A5.1 仅回归 trunk | 0.775±0.001 | 0.584±0.006 | 1.400±0.010 | 1.817±0.001 | 0.062±0.025 |
| A5.2 回归+分类 trunk | 0.777±0.001 | 0.590±0.003 | 1.389±0.006 | 1.792±0.013 | 0.052±0.013 |
| A5.3 回归+排序 trunk | 0.776±0.004 | 0.592±0.004 | 1.387±0.007 | 1.790±0.011 | 0.020±0.014 |
| A5.4 三任务 trunk | 0.778±0.001 | 0.599±0.005 | 1.375±0.008 | 1.770±0.017 | 0.032±0.029 |
| A5.5 Learned weights | 0.782±0.001 | 0.600±0.002 | 1.372±0.004 | 1.767±0.005 | 0.036±0.037 |

### Tier 1 Go/No-Go Gate (A5.2 vs A5.0)

- **(a)** ρ(A5.2)=0.777 ≥ ρ(A5.0)−0.01=0.770 → **PASS**
- **(b)** NLL(A5.2)=1.792 ≤ NLL(A5.0)+0.05=1.836 → **PASS**
- **(c)** ρ gain=−0.002 OR ρ_{|err|,σ} gain=−0.039 → **FAIL**
- **Decision: NO-GO**

### 关键 Takeaways

1. **Trunk shaping 系统性降低 ρ_{|err|,σ}**: 从 baseline 的 0.091 降到 0.020–0.062，是根本性失败。trunk 破坏了 DKL Ensemble 的 error-uncertainty correlation。
2. **A5.5 (learned weights) 是唯一在 ρ 上持平/微超 baseline 的变体** (0.782 vs 0.780)，NLL 也有改善 (1.767 vs 1.786)，但 ρ_{|err|,σ} 仍然退化。
3. **点预测影响有限**: ρ 在 0.775–0.782 范围内波动，trunk 既不能显著改善也不显著恶化点预测。
4. **训练动态**: 固定权重变体极早 early-stop (epoch 1–5)，learned weights 可训练更久 (epoch 19–25)，说明固定权重导致任务冲突快速发散。
5. **结论**: 128 维 frozen diffusion embedding 已经足够好，额外 MLP trunk 只会稀释而非增强信号。SP04 DKL Ensemble 直接消费 frozen embedding 是最优配置。

### 生成的 Artifacts

- `results/stage2/multitask_trunk/A5.{0..5}_*/seed{42,123,777}/eval_results.json`
- `results/stage2/multitask_trunk/figures/` — comparison_table.json, training curves PDF ×6, tsne_trunk.pdf
- `results/stage2/oracle_heads/frozen_embeddings_augmented.npz` — 增强 NPZ (含 codes + groups)

### Next: Sub-Plan 06 (Physics-Aware Features)
